...
- [x] `MainWindow`：顶层就是一个 `QTabWidget`（Hex / Terminal 两个 Tab，不再套一层"Receive"分组），两个视图都全程订阅同一个 Session，不因为 Tab 不在前台就停止更新。（M3 起这个 Tab widget 挪进了 `SessionPanel`，见 M3 章节——`MainWindow` 这个类名此后指的是外层的会话 Tab 管理窗口。）
...
**macOS 打包发布事故（v0.2.1）**：为了发一个"不用编译直接能跑"的可执行文件，用 `macdeployqt` 把 Qt 的 framework/插件拷进 `.app`（`install_name_tool` 顺带把里面的库路径从开发机的 Qt SDK 绝对路径改写成 `@executable_path/../Frameworks`）。拷贝+改写之后本机双击能跑，但从 GitHub Release 下载下来的压缩包解压后打不开，报"已损坏，应该移到废纸篓"。根因：CMake/Xcode 工具链在链接时已经给主执行文件盖了一个 ad-hoc 签名（`codesign -dv` 能看到 `flags=0x20002(adhoc,linker-signed)`），但那个签名是在 `macdeployqt` 往 `.app` 里塞 Frameworks/PlugIns **之前**盖的——`codesign --verify --deep --strict` 报错 `code has no resources but signature indicates they must be present`，意思是签名记录的包内容和现在的包内容对不上（多出来一堆 framework）。本机双击不报错是因为本机运行不走完整的 Gatekeeper 校验路径，但下载后的文件带着 `com.apple.quarantine` 标记，一走完整校验立刻露馅。**修复**：`macdeployqt` 跑完之后必须重新对整个 `.app` 做一次深度 ad-hoc 签名——`codesign --force --deep --sign - SerialKit.app`——把签名和当前包内容重新对齐，`codesign --verify --deep --strict` 应该干净通过（返回码 0）才能打包发布，验证过给 `.app` 打上模拟下载用的 `com.apple.quarantine` 标记后依然能正常启动（走的是 App Translocation，不是硬拦截）。这个签名是 ad-hoc 的，不是 Apple Developer ID + 公证，`spctl -a --type execute` 还是会 `rejected`——这是预期的，双击/右键打开时用户还是会看到一次"身份不明的开发者"提示，ad-hoc 签名只解决"损坏打不开"这个硬性阻断，不能让 Gatekeeper 完全放行（那需要付费 Developer ID 证书 + notarize，不在当前范围内）。以后每次用 `macdeployqt` 重新打包发布，这一步"重新签名"都不能漏。

### M3：多会话（多 Tab）+ 原始日志 UI

M3 原定范围"多命令宏 + 原始日志 + 会话管理"里，"多命令宏"已经在 M1 提前做完（见上方说明），实际只剩两块：`RawLogger` 一直没有 UI 开关，`SessionManager` 内部本来就没有数量上限（`vector<unique_ptr<Session>>`），"只能有一个连接"完全是旧版 `MainWindow::toggleConnection()` 的 UI 逻辑造成的——放开限制的工作量几乎全在 UI 层，`Session`/`SessionManager` 内部结构不用动。

- [x] `SessionManager::removeSession(Session*)`：新增按指针删除的重载，`removeSession(int)` 保留不动。按指针删除是因为 `SessionPanel` 不追踪自己在 `SessionManager` 内部 vector 的位置——其它 Tab 关闭会让索引漂移，追踪索引本身就是一种隐患。
- [x] `src/view/session_panel.h/.cpp`（新增）：把旧版 `MainWindow` 构造函数里"一个连接该有的一切"整体搬过来——连接栏、Hex/Terminal 顶层 Tab（M2 的结构原样保留）、`MultiLineSend`、`MacroPanel`，外加新的日志开关。每个会话 Tab 对应一个实例，直接持有 `SessionManager*`（非拥有），连接/断开时自己调用 `addSession`/`removeSession`。
- [x] 原始日志开关：一个"Start Logging..."按钮 + 只读 `QLineEdit` 显示当前日志路径（复用文件发送那边"只读 QLineEdit 显示路径"的约定，见下方 UI 交互约定第 9 条）。点击调用 `session->rawLogger()->start(path)`（`QFileDialog::getSaveFileName` + `DontUseNativeDialog`，同样是踩过坑的既有约定）。
- [x] `MainWindow` 改造：不再直接持有任何"一个连接"相关的控件，只管理外层 `QTabWidget`（`setTabsClosable(true)` + `setCornerWidget()` 放一个"+"按钮）。默认启动预置 1 个 `SessionPanel` Tab，不是空白 0 个。
- [x] `tests/test_raw_logger.cpp`、`tests/test_session_manager.cpp`（新增，无头，`serialkit_headless`）：`RawLogger` 的 start/stop/isLogging 状态机 + 挂到真实 `RingBuffer` 上验证写入内容格式；`SessionManager` 的按索引/按指针删除、信号触发、越界/未知指针的 no-op。
- [ ] 真实多设备场景验证（同时接两块板子，确认互不干扰、日志文件内容正确）——需要用户配合实测，见本节末尾。

**M3 范围内的几个设计决定**：
- **状态栏从 `QMainWindow::statusBar()` 搬进每个 `SessionPanel` 内部**：旧版状态栏是全局唯一的，一旦允许多个会话同时存在，"全局唯一的一条状态栏"就没法表示"这是哪个会话的状态"——连接状态文字和收发字节计数现在是每个 `SessionPanel` 自己布局里的一行，样式和逻辑照搬旧版，只是不再挂在 `QMainWindow` 上。
- **日志状态不跨重连保留**：断开或重连都是一个新 `Session`/新 `RawLogger`（`Session` 构造时才 `new` 一个），旧的日志文件路径和"正在记录"状态在断开时被显式清掉（`SessionPanel::disconnectSession()`），要接着记就重新点一次"Start Logging..."——不做"记住上次的文件路径，重连自动继续写"这种隐式行为，避免用户以为还在记录、其实文件早就没人写了。
- **宏面板每个 Tab 一份 widget 实例，但 `QSettings` 配置全局共享**：`MacroPanel` 的持久化 key 本来就是扁平、不带会话 ID 的（`macros/slot0/...`），所以多个 `SessionPanel` 各自的 `MacroPanel` 实例天然共享同一份配置内容——这是有意的，用户不用给每个会话 Tab 重新配一遍宏。唯一的限制：**不做实时跨 Tab 同步**，Tab A 改了宏内容，Tab B 已经在内存里的 `MacroPanel` 不会自动刷新（要等下次重建，比如整个 App 重启）。这是已知、可接受的限制，YAGNI，真需要实时同步再加。
- **默认预置 1 个会话 Tab，允许关到 0 个**：预置 1 个是为了不打破 M1/M2 建立的"打开就能直接看到连接栏"体验；关闭最后一个 Tab 不做特判阻止，允许变成 0 个 Tab（界面上只剩"+"按钮），保持行为一致、不为一个边界情况单独加逻辑。
...
7. **面板默认折叠，用一个按钮展开/收起**：`MacroPanel` 默认 `hide()`，宿主用一个"▸/▾ Macros"按钮切换可见性（M1/M2 时宿主是 `MainWindow`，M3 起是 `SessionPanel`，逻辑没变，只是搬了个类）——这是用户特别认可的 SSCOM"隐藏"按钮交互，不要改成 Tab 切换或者始终展开。
...
10. **状态行左边讲状态、右边讲统计，字节计数每次新连接清零**：连接状态文字（`m_statusLabel`）和收发字节计数（`m_byteCountLabel`）左右分布在同一行里。**M3 变更**：这一行原来挂在 `QMainWindow::statusBar()` 上（全局唯一），M3 允许多个会话并存后，"全局唯一状态栏"没法表达"这是哪个会话的状态"，改成挂在 `SessionPanel` 自己的布局里，每个会话 Tab 一份——布局摆法（左状态右计数）和清零时机（`toggleConnection` 连接成功时）不变，只是不再是 `QMainWindow` 的一部分。断开后保留数字直到下次连接，方便用户看这次会话总共收发了多少。
...
16. **Hex/Terminal 提到最外层 Tab，Send 区域只存在于 Hex 页里，不是靠联动隐藏**：一开始的方案是 Hex/Terminal 当 Receive 分组内部的子 Tab，Send 分组作为兄弟节点靠 `currentChanged` 信号联动隐藏/显示；讨论后改成更直接的做法——`Hex`/`Terminal` 直接是"最外层"（M2 时是 `MainWindow` 的 `QTabWidget`；M3 起 `MainWindow` 的最外层 Tab 变成会话列表，Hex/Terminal 降一级变成每个 `SessionPanel` 内部的 `QTabWidget`，但相对"一个会话的 UI"来说仍然是它的最外层，下面这条决定完全不受影响）两个 Tab，Hex 的 Tab 页自己装一个"Receive"分组（`HexView`）+ 一个"Send"分组（发送框、Macros）；Terminal 的 Tab 页就是裸的 `TerminalView`，不套"Receive"分组框（Tab 标题已经说明是 Terminal 了，不需要再嵌套一层同义的标题），也没有 Send 区，把整页空间都让给终端。两种做法数据层面等价（见上面第 13 条），选后者纯粹是因为心智模型更直接：用户想的是"我现在是 Hex 模式还是 Terminal 模式"，而不是"Receive 里选了哪个子 Tab、Send 会不会跟着变"。**踩过的坑**：字体缩放这条改动让"改变行列数导致回滚缓冲区里存的行比当前列数窄"这件事从理论情况变成了会实际触发的路径——`VtermEngine::handleSbPopLine` 原来只写 `cells[i] = line.at(i)`（`i < line.size()` 才写），列数变宽后 `cells` 数组后半段留着未初始化的内存，libvterm 的 `resize()` 内部靠 `cells[col].width` 步进读这块内存，宽度如果凑巧是垃圾值 0 就会死循环——实测直接把主线程卡死（`sample` 抓到的调用栈钉死在 `resize_buffer`/`screen.c:699` 那个 `for` 循环里出不来）。修复：补齐的空位显式填一个 `width=1` 的空白 cell，不留未初始化内存。这是"频繁改变终端行列数"第一次被真正跑到的代码路径，如果以后有别的地方也会动态改 `VtermEngine::resize()` 的行列数，先想一下会不会踩到同一类"缓冲区/图层大小不匹配"的坑。
17. **端口选择只显示"能用的"，macOS 上过滤掉 `tty.*` 和几个系统虚拟端口**：`SerialTransport::availablePortNames()` 内部先拿 `QSerialPortInfo::availablePorts()` 的原始名字，再过一遍 `SerialTransport::filterUsablePortNames()`。macOS/BSD 给每个真实串口设备都注册两个节点——`tty.*`（呼入，`open()` 会等 DCD 信号阻塞）和 `cu.*`（呼出，不等）——USB 转串口场景基本只会用 `cu.*`，把 `tty.*` 留在下拉框里纯粹是让用户有一半概率选错。另外 `cu.debug-console`（Apple Silicon 内置调试 UART）和 `cu.Bluetooth-Incoming-Port`（蓝牙虚拟端口）是所有 Mac 都会有的系统端口，不是真实设备，也一并从黑名单里去掉。这条规则**只在 macOS 上生效**（`#ifdef Q_OS_MACOS` 编译期隔离）——Linux（`/dev/ttyUSB0`）、Windows（`COM3`）根本没有 `cu`/`tty` 这套命名，其他平台上 `filterUsablePortNames()` 是纯透传。过滤逻辑单独拆成一个不依赖真实硬件的静态函数，方便喂假数据做无头单元测试（`tests/test_serial_transport.cpp`），不用连真实设备就能验证过滤规则本身对不对。
