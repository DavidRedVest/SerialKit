...
## 里程碑范围（M1、M2 已完成并通过用户验证，当前在 M3 — 严格执行，禁止范围蔓延）
...
M2（已完成）：接入 libvterm，实现"同一份字节流的两个视图"（Hex + Terminal 顶层 Tab）、Terminal 字体缩放、macOS 端口列表过滤。发布为 v0.2.1，细节见 `docs/ARCHITECTURE.md` M2 章节和"UI 交互约定"第 11-17 条。

M3（当前）目标：原始日志的 UI（`RawLogger` 在 M1 就写好了但一直没有开关）+ 多会话/多 Tab（放开 `SessionManager` 恒为 1 的限制）。"多命令宏"已在 M1 提前做完，不在 M3 范围内重做。

**只做**：
- `SessionPanel`（`src/view/session_panel.h/.cpp`）：把原来 `MainWindow` 里"一个连接该有的一切"（连接栏、状态行、Hex/Terminal Tab、Send、宏面板）搬进这个类，每个会话 Tab 一个实例
- `MainWindow` 改成外层 `QTabWidget`，管理若干个 `SessionPanel`，"+" 新增、Tab 可关闭，默认预置 1 个 Tab
- `RawLogger` 的开始/停止 UI（文件选择 + 按钮），挂在 `SessionPanel` 里
- `SessionManager::removeSession(Session*)`（按指针删除，配合 Tab 关闭时索引会漂移的场景）

**禁止在 M3 引入**：QCustomPlot 波形视图、QJSEngine 脚本引擎、TCP/UDP 传输、任何协议解码插件的具体实现（`IProtocolDecoder` 仍只有 `RawPassthroughDecoder`）、宏面板的跨 Tab 实时同步（配置本身通过 QSettings 全局共享，但不做"编辑一个 Tab 立刻刷新其它 Tab 界面"的机制，见 `docs/ARCHITECTURE.md` M3 章节）。
...
4. **Session 是组合根**：新增功能时，判断它属于 `Session`（每个会话独立的状态：Transport/RingBuffer/Framer/RawLogger/视图订阅）还是 `SessionManager`（跨会话的管理，如会话列表）还是 `SessionPanel`（M3 起：单个会话对应的 UI，连接栏/Hex-Terminal Tab/Send/宏面板/日志按钮）。不要把本该属于 Session 的状态提升成全局单例——M3 已经放开多会话，`SessionManager` 内部一直没有数量上限，这个约束一直是真的。
